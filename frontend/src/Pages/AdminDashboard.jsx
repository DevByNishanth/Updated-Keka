...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const results = leaveRequest.filter((item) =>
      item.user_name.toLowerCase().includes(searchQuery.toLowerCase())
    );
    setFilteredRequests(results);
  }, [searchQuery, leaveRequest]);
  ...
  const filteredLeaveRequests = adminLeave.filter((leave) => {
    if (filter === "")
      return leave.status === "Rejected" || leave.status === "Approved"; // Show all
            ...
            <div className="leave-request-container mt-4 grid gap-3 sm:grid-cols-2 md:grid-cols-4 h-fit overflow-auto pr-2">
              {filteredRequests.length > 0 ? (
                filteredRequests.map((item, index) => (
                  <div className="card border rounded-lg p-3 h-fit" key={index}>
                ...
                <div className="no-results text-center mt-4 text-xl">
